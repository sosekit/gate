...
function createGeometry(shape) {
  const geometries = {
    "canvas-portrait": () => new THREE.BoxGeometry(1.3, 1.72, 0.1, 8, 8, 2),
    "canvas-square": () => new THREE.BoxGeometry(1.52, 1.52, 0.1, 8, 8, 2),
    "canvas-landscape": () => new THREE.BoxGeometry(1.8, 1.2, 0.1, 8, 8, 2),
    "canvas-tall": () => new THREE.BoxGeometry(1.08, 1.9, 0.1, 8, 8, 2),
    "canvas-wide": () => new THREE.BoxGeometry(1.95, 1.06, 0.1, 8, 8, 2)
  };

  return (geometries[shape] || geometries["canvas-portrait"])();
}
...
if (carousel) {
  ...
  const products = window.FOOLS_GATE_CONTENT?.products?.length
    ...
    : [{ name: "sdjhkdhfd", price: "$00.00", description: "jshdf kjshdfk dhfksj.", shape: "canvas-portrait", color: "#f8f7f2", roughness: 0.72, metalness: 0.02 }];
  ...
  if (renderer) {
    ...
    const objects = products.map((product, index) => {
      ...
      const shadowCatcher = new THREE.Mesh(
        ...
        new THREE.ShadowMaterial({ color: 0xa9acb6, opacity: 0.1, transparent: true, depthWrite: false })
      );

      mesh.rotation.set((index - 1) * 0.04, (index - 1) * 0.08, (index - 1) * 0.025);
      mesh.castShadow = true;
      shadowCatcher.position.z = -0.2;
      shadowCatcher.receiveShadow = true;
      group.add(shadowCatcher, mesh);
      group.userData = {
        mesh,
        phase: index * 2.1,
        targetX: 0,
    ...
    function animate() {
      ...
      objects.forEach((object) => {
        ...
        if (!reducedMotion) {
          ...
          data.mesh.rotation.x = Math.sin(elapsed * CAROUSEL_SETTINGS.tiltSpeed + data.phase) * CAROUSEL_SETTINGS.tiltX;
          data.mesh.rotation.y = Math.sin(elapsed * CAROUSEL_SETTINGS.tiltSpeed * 0.83 + data.phase * 0.7) * CAROUSEL_SETTINGS.tiltY;
          data.mesh.rotation.z = Math.sin(elapsed * CAROUSEL_SETTINGS.tiltSpeed * 0.67 + data.phase * 1.2) * CAROUSEL_SETTINGS.tiltZ;
        }
      });
