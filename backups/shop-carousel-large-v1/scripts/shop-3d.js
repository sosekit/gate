...
if (carousel) {
  ...
  const products = window.FOOLS_GATE_CONTENT?.products?.length
    ...
    : [{ name: "Object", price: "$00.00", description: "Placeholder object.", shape: "sphere", color: "#ffffff", roughness: 0.2, metalness: 0.5 }];
  ...
  if (renderer) {
    ...
    function setTargets(immediate = false) {
      objects.forEach((object, index) => {
        ...
        data.targetScale = depth === 0 ? 1 : CAROUSEL_SETTINGS.inactiveScale;
        data.targetOpacity = depth === 0 ? 1 : CAROUSEL_SETTINGS.inactiveOpacity;
