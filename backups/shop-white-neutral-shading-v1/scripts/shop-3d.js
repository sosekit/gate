...
if (carousel) {
  ...
  if (renderer) {
    ...
    scene.add(new THREE.HemisphereLight(0xffffff, 0xf0f0f4, 2.2));
    ...
    const rimLight = new THREE.PointLight(0xcfdfff, 18, 18, 2);
    rimLight.position.set(-4.5, 1.2, 3.2);
    scene.add(rimLight);

    const lavenderLight = new THREE.PointLight(0xe5d9ff, 12, 16, 2);
    ...
    const objects = products.map((product, index) => {
      ...
      const material = new THREE.MeshPhysicalMaterial({
        color: product.color,
        metalness: product.metalness * 0.45,
        roughness: Math.max(product.roughness, 0.3),
